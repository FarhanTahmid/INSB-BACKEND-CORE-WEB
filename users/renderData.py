...
from central_branch.models import Events,Type_of_Event
...
def getTypeOfEventStats():
    ...
    event_stats_keys =[]
    event_stats_values=[]
    all_event_type=Type_of_Event.objects.all()
    all_events_number = Events.objects.all().count()
    event_percentage ={}
    for i in all_event_type:
        event_count = Events.objects.filter(type_of_event = i.pk).count()
        try:
